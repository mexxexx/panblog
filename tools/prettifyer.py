...
def prettify_html(html_string, debug=False):
    ...
    sup_end_space = RegExSub(r"[\n\s]*(</sup>)([a-zA-Z0-9])", re.S, r"\g<1> \g<2>")
    ...
    # Inlining code tags not lead by pre tag
    inline_code_start = RegExSub(
        r"(?<!<pre>)[\n\s]*(<(code).*?>)[\n\s]*", re.S, r" \g<1>"
    )
    ...
    # Removes whitespace between end of inline code tags and beginning of new tag
    inline_code_end = RegExSub(
        r"[\n\s]*(</(code)>)[\n\s]*(?!<\/pre>)(?=<)", re.S, r"\g<1>"
    )

    # Adds a space between the ending inline tags and following words
    inline_space = RegExSub(
        r"[\n\s]*(</(strong|a|span)>)([a-zA-Z0-9])", re.S, r"\g<1> \g<3>"
    )

    # Adds a space between the ending inline code tags and following words
    inline_code_space = RegExSub(
        r"[\n\s]*(</(code)>)(?![\n\s]*<\/pre>)([a-zA-Z0-9])", re.S, r"\g<1> \g<3>"
    )
