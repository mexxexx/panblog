    ...
    if tags:
        ...
        for tag in tags:
            result += "<li>[{0}]({1}{0}.md)</li>\n".format(
                tag, config["GENERAL"].get("BlogTagDirectory", "/")
            )
        result += "</ul>\n\n"
...
def create_blog_preview(posts, page_num, last_page, max_word_count, config):
    ...
    if not page_num == 1:
        result += "[&#171; {0}]({1}page{0}.md){{.prev_blog_page}}\n".format(
            str(page_num - 1), config["GENERAL"].get("BlogPagesDirectory", "/")
        )
    ...
    for page_num, content in pages:
        filenames = [
            "{0}/{1}page{2}.md".format(
                build_directory,
                config["GENERAL"].get("BlogPagesDirectory", "/"),
                page_num,
            )
        ]
        if page_num == 1:
            filenames.append(
                "{0}/{1}index.md".format(
                    build_directory, config["GENERAL"].get("BlogDirectory", "/")
                )
            )
...
def write_tag_pages(tag_pages, build_directory, config):
    for tag, content in tag_pages:
        filename = "{}/{}{}.md".format(
            build_directory, config["GENERAL"].get("BlogTagDirectory", "/"), tag
        )
